    ...
    if clan:
        ...
        if user_id == clan_owner_id:
            ...
            if reply_msg:
                ...
                cursor.execute('SELECT clan_member, clan_invite FROM users WHERE id = ?', (invited_user_id,))
                user = cursor.fetchone()
                clan_member = user[0] if user[0] is not None else 0
                clan_invite = user[1] if user[1] is not None else 0
                
                if not user:
                    cursor.execute('INSERT INTO users (id, drug_count, is_admin, is_banned, clan_member, clan_invite) VALUES (?, 0, 0, 0, 0, 0)', (invited_user_id,))
                    conn.commit()
                if clan_member == 0 and clan_invite == 0:
                    cursor.execute('UPDATE users SET clan_invite = ? WHERE id = ?', (clan_id, invited_user_id))
                    conn.commit()
...
async def clan_accept(callback_query: CallbackQuery):
    ...
    if int(user_id) == int(invited_user_id):
        cursor.execute('UPDATE users SET clan_member = ?, clan_invite = 0 WHERE id = ?', (clan_id, user_id))
        conn.commit()
